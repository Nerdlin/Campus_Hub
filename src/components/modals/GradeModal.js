...
const GradeModal = ({ isOpen, onClose, student, subject, onSaveGrade }) => {
  const { t } = useTranslation();
  const [gradeData, setGradeData] = useState({ value: '', type: '', date: '', semester: '', subjectCode: '', credits: '' });
  const [saving, setSaving] = useState(false);

  useEffect(() => {
    // Сброс формы при открытии для нового студента/предмета или при закрытии
    if (isOpen) {
      setGradeData({ value: '', type: '', date: new Date().toISOString().slice(0, 10), semester: '', subjectCode: '', credits: '' });
    } else {
  ...
  const handleSave = async (e) => {
    e.preventDefault();
    if (!student || !subject) return; // Проверка наличия данных

    setSaving(true);
    try {
      // Передаем все необходимые данные для сохранения оценки
      await onSaveGrade(student.id, { ...gradeData, subject: subject.name });
      // onClose(); // Закрываем модалку после успешного сохранения
    } catch (error) {
      console.error('Error saving grade:', error);
      // Обработка ошибки, возможно, через toast
    } finally {
      setSaving(false);
    }
  };
            ...
            >
              <Dialog.Panel className="w-full max-w-md transform overflow-hidden rounded-2xl bg-white p-6 text-left align-middle shadow-xl transition-all">
                <Dialog.Title as="h3" className="text-lg font-medium leading-6 text-gray-900 flex justify-between items-center">
                  {t('Grade')} {student?.name} {t('in')} {subject?.name}
                  <button
                    type="button"
